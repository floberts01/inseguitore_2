...
from servomove_firmata import servopos
import argparse
import signal
import time
import sys
...
if __name__ == "__main__":
	...
	with Manager() as manager:
		...
		panP = manager.Value("f", 1)#0.09
		panI = manager.Value("f", 0)#0.08
		panD = manager.Value("f", 0)#0.002

		# set PID values for tilting
		tiltP = manager.Value("f", 1)#0.11
		tiltI = manager.Value("f", 0) #0.10
		tiltD = manager.Value("f", 0)#0.002
